...
if __name__ == "__main__":
	...
	feature_imps = runRF(train, train_y)
	select_indices = np.argsort(feature_imps)[-1000:]
	train = getSelectIndices(train, select_indices)
	print train.shape
	model, sc = runNN(train, train_y)
	...
	test = getTestData()
	print test.shape
	test = getSelectIndices(test, select_indices)
	print test.shape
	test_id = pd.read_csv("../Data/patients_test.csv").patient_id.values
	test_preds = predictNN(test, model, sc)

	preds_df = pd.DataFrame({'patient_id':test_id, 'predict_screener':test_preds})
	preds_df.to_csv("sub_nn_withencode2_srk.csv", index=False)
