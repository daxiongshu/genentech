            ...
            for col in [0.4]:
                for sub in [1]:
                    for num in [2000]:
                        ...
                        clf.fit(X,y)
                        ypred=clf.predict_proba(Xt).T[1]
                        #clf=xgb_classifier(eta=eta,min_child_weight=child,depth=depth,num_round=num,col=col,subsample=sub,seed=seed)
                        #ypred=clf.train_predict(X,y,Xt,yt)        
                        #s=pd.DataFrame({'patient_id':idx,'predict_screener':ypred})     
                        #s.to_csv('xgb7.csv',index=False)           
                        score=myauc(yt,ypred)
                        tmpname='%s_eta_%f_depth_%d_child_%d_col_%f_sub_%f_num_%d_seed_%d_score_%f'% (myname,eta,depth,child,col,sub,num,seed,score)
                        print 'myauc',score,tmpname
                        s=pd.DataFrame({'patient_id':idx,'is_screener':ypred,'real':yt})
                        s.to_csv('sgdcv.csv',index=False)
